...
public static string ApplicationName = "Tarsvin";
public static string BuildConfig = "Release";
public static string BuildPlatform = "Any CPU";
public static string CompanyName = "Darshit Dave";
...
public static string NuGetToolsPath = ApplicationPath + @"\Tarsvin.Build\tools\Tarsvin";

public static string SolutionFile = "Tarsvin.sln";
...
public static string[] DistributeAppPaths = new string[] { ApplicationPath };

public static string DotNetVersion = "net451";
...
public static void Build(string version, string configuration = "Release", string platform = "Any CPU")
{
    ...
    string cmd = string.Format("{0} /p:Configuration={1} /t:Rebuild /m", SolutionFile, configuration, OutputPath);

    Log.Info("Build Command: " + cmd);
    ...
    try
    {
        ...
        foreach (var path in filePaths)
        {
            ...
            AssemblyInfo assemblyInfo = new AssemblyInfo()
                ...
                .Attribute("AssemblyCompany", CompanyName)
                .Reference("System")
...
public static void PackageProject(string nuspec, string version, string output, string basePath)
{
    ...
    string packCmd = string.Format("pack \"{0}\" -Version {1} -OutputDirectory \"{2}\" -BasePath \"{3}\" -NoPackageAnalysis", nuspec, version, output, basePath);

    Log.Info("Pack Command: " + packCmd);

    Exec(NuGetExe, packCmd);
}

public static void NuGetRestore()
{
    string cmd = string.Format("restore {0} -source \"http://agpjaxsrvbuild2/pne.nuget/nuget\"", SolutionFile);
...
public static void CopyLibFiles(string directory, string projectName)
{
    ...
    string dest = string.Format(@"{0}\{1}\lib\{2}", DistroPath, projectName, DotNetVersion);
